...
app.get('/user/:id', (req, res) => {
  ...
  if (param === '' || param === undefined || !Number.isInteger(+param)) {
    return res.status(404).send('<h1>404 Ресурс не найден</h1>');
  }

  const userJson = JSON.parse(fs.readFileSync('public/users.json'));
  const result = userJson.filter((el) => el.id == param);

  if (result[0] === undefined) {
    return res.status(405).send('<h1>405 Метод недоступен</h1>');
  }
