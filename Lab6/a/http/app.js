...
let HTTP404 = (req, res) =>{
    ...
    res.end('<h1>404 Не найден ресурс</h1>');
}
let HTTP405 = (req, res) =>{
    res.writeHead(400, { 'Content-Type': 'text/html;charset=utf-8' });
    res.end('<h1>405 Метод не доступен</h1>');
}
// /connection?set=set
...
let user = (req, res) =>{
    ...
    let userJson = JSON.parse(fs.readFileSync('public/users.json'));
...
let formparameterGET =  (req, res) =>{
    res.end(fs.readFileSync('6.html'));
}
...
// /json
let jsonGET =  (req, res) =>{
    res.end(fs.readFileSync('7.html'));
}
...
let files = (req, res) => {
    ...
    let FilePath = `${__dirname}\\public\\${SplitUrl[2]}`;  
...
let get_upload =  (req, res) =>{
    res.end(fs.readFileSync('9.html'));
}
